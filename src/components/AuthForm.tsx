    ...
    <main className="mx-auto flex w-full max-w-sm flex-1 flex-col justify-center gap-6 px-6 py-16">
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-4">
        <div>
          ...
          <input
            ...
            autoComplete="email"
            {...register("email")}
            className="mt-1 w-full rounded-xl border border-zinc-300 bg-white px-3 py-2 text-sm outline-none focus:border-brand-900"
          />
          {errors.email && (
        ...
        <button
          ...
          className="mt-1 rounded-xl bg-brand-900 px-4 py-2 text-sm font-medium text-white hover:bg-brand-700 disabled:opacity-50"
        >
          {isSubmitting
            ? "처리 중…"
            : isRegister
      ...
      <p className="text-center text-sm text-zinc-500">
        {isRegister ? (
          <>
            이미 계정이 있으신가요?{" "}
            <Link href="/login" className="font-medium text-brand-900 underline">
              로그인
            </Link>
          </>
        ) : (
          <>
            계정이 없으신가요?{" "}
            <Link href="/register" className="font-medium text-brand-900 underline">
              회원가입
            </Link>
          </>
        )}
